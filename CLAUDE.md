...
SwiftUI, iOS 17+, generated by XcodeGen. Scoped to the `/add` session editor — no history browsing,
no PR trophies, no last/working-max stats, so it never loads the full lifting log. Only the exercise
catalog (`exercise-classified_{ts}.json.gz`, disk-cached) and the Firestore write.
...
- `Sources/Services/` — auth, Firestore write, catalog fetch, draft file store, Firebase bootstrap
- `Sources/State/SessionModel.swift` — the session reducer; the draft is one Codable blob whose `didSet` writes through
- `Sources/Views/` — readiness → live → done, plus the detail and picker sheets
...
- **The Swift side duplicates contracts by hand** — nothing generates them, so a change on one side silently rots the other. When you touch any of these, touch both:
  ...
  |---|---|
  | `Models/LiftingWorkoutDoc.swift` | `LiftingWorkoutDoc` in `apps/web/src/lib/firebase.ts`, decoded by `apps/data/src/firestore/download.ts` |
