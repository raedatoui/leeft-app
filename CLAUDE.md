...
SwiftUI, iOS 17+, generated by XcodeGen. Two tabs: the `/add` session editor, and a History
tab that reads `lifting-workouts` back. No PR trophies, no last/working-max stats — it never
loads the full lifting log, only the exercise catalog (`exercise-classified_{ts}.json.gz`,
disk-cached) and the Firestore collection it writes to. History therefore shows app-logged
days only, not the TrainHeroic years that live in the CDN artifacts.
