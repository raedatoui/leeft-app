...
export const getTopSet = (workout: MappedWorkout, repRange?: RepRange): SetDetail | undefined => {
    ...
    const filteredSets = workout.selected.sets.filter((s) => s.reps && s.reps >= (repRange?.min ?? 1) && s.reps <= (repRange?.max ?? 5));

    return filteredSets.length > 0 ? filteredSets.sort((a, b) => b.weight - a.weight)[0] : undefined;
};

export const getVolumeWeight = (workout: MappedWorkout): number => {
    if (!workout?.selected?.sets) return 0;
    return workout.selected.sets.reduce((acc, s) => acc + s.weight * (s.reps ?? 0), 0);
};

export const getMaxWeight = (workout: MappedWorkout, repRange?: RepRange): number => {
    const set = getTopSet(workout, repRange);
    return set?.weight ?? 0;
};

export const getTopNSets = (workout: MappedWorkout, setCount = 3, targetReps = 5): number => {
    if (!workout?.selected?.sets) return 0;
    ...
    const maxWeightForTargetReps = Math.max(...targetRepSets.map((set) => set.weight));

    // Find the index of the set with max weight for target reps
    const maxWeightIndex = sets.findIndex((set) => set.reps === targetReps && set.weight === maxWeightForTargetReps);

    // Check all possible windows that could include the max weight set
    for (let start = Math.max(0, maxWeightIndex - setCount + 1); start <= Math.min(maxWeightIndex, sets.length - setCount); start++) {
...
const createOneRepMaxCalculator = (oneRepMax: OneRepMaxCalc) => (workout: MappedWorkout, repRange?: RepRange) => {
    const set = getTopSet(workout, repRange);
...
export const oneRepMaxCalculators: CalculationMethod[] = [
    ...
    },
    {
        ...
        calculator: createOneRepMaxCalculator((w, r) => (100 * w) / (101.3 - 2.67123 * r)),
    },
    {
        name: 'Lombardi',
    ...
    },
    {
        ...
        calculator: createOneRepMaxCalculator((w, r) => (100 * w) / (52.2 + 41.9 * Math.exp(-0.055 * r))),
    },
    {
        name: "O'Conner",
    ...
    },
    {
        ...
        calculator: createOneRepMaxCalculator((w, r) => (100 * w) / (48.8 + 53.8 * Math.exp(-0.075 * r))),
    },
];

/*
export const epley = (workout: MappedWorkout, repRange?: RepRange): number => {
    const set = getTopSet(workout, repRange);
    if (!set?.reps) return 0;
...
export const lombardi = (
    ...
    const set = getTopSet(workout, repRange);
    if (!set?.reps) return 0;
    return set.weight * Math.pow(set.reps, 0.1);
};

export const mayhew = (workout: MappedWorkout, repRange?: RepRange): number => {
    const set = getTopSet(workout, repRange);
    if (!set?.reps) return 0;
    return (100 * set.weight) / (52.2 + 41.9 * Math.exp(-0.055 * set.reps));
};

export const oconner = (
    workout: MappedWorkout,
    repRange?: RepRange
): number => {
    const set = getTopSet(workout, repRange);
...
export const wathen = (workout: MappedWorkout, repRange?: RepRange): number => {
    const set = getTopSet(workout, repRange);
    if (!set?.reps) return 0;
    return (100 * set.weight) / (48.8 + 53.8 * Math.exp(-0.075 * set.reps));
};
 */
